...
public class BinarySearchTree<E extends Comparable<E>> extends AbstractTree<E> {
   ...
   * Check if each node is a leaf; if it is a leaf, incriment the count.
   * Is inOrder search used?
   * @return the count.
   */
  ...
  /**check if tree is empty
   * Create an ArrayList
   * Iterate through the tree in preorder and return each left element starting from specified element.
   * Return the ArrayList otherwise.
   ...
   */
    ...
    //left for you to implement in Lab 7
      ...
      if(root != null && search(e)) {
        TreeNode<E> current = root.left;//Start from the left of the root.
        current = current.left;
        current = current.right; // does this only check the right node?
    ...
    public ArrayList<E> rightSubTree(E e) {
      ...
      java.util.ArrayList<E> list = new java.util.ArrayList<>();
      if(root != null && search(e)) {
        TreeNode<E> current = root.right;//Start from the right of the root.
        current = current.left;
        current = current.right;
        if (current.element == null)
          list.add(current.element);
  ...
  /* Returns the inorder predecessor of the specified element, returns null if tree is empty or element 'e' is not in the tree. */
    ...
    //left for you to implement in Lab 7
      ...
      TreeNode<E> current = root;
        current = current.left;
        current = current.right;
        inorderPredecessor(current.element);
      } else {
